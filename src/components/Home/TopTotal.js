...
const TopTotal = (props) => {
  ...
  let countIsPaid = 0;
  let countIsNotPaid = 0;
  if (orders) {
    orders.map((order) =>
      order.is_paid === true ? (totalSale = totalSale + order.total_price) : null
    );
  }
  if (orders) {
    orders.map((order) =>
      order.is_paid === true ? (countIsPaid += 1) : null);
  }
  if (orders) {
    orders.map((order) =>
      order.is_paid === false ? (countIsNotPaid += 1) : null);
  }
  if (orders) {
    orders.map((order) =>
      totalSaleFuture = totalSaleFuture + order.total_price);
  }
